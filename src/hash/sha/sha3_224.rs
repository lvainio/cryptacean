...
fn pad(input: &mut Vec<u8>) -> Vec<u64> {
    ...
    if input_length % 144 == 143 {
        input.push(0x86); // 1000 0110
    } else {
        let num_zero_bytes = 144 - (input_length % 144) - 2;
...
impl HashFunction for SHA3_224 {
    fn hash(&self, input: &Input) -> Output {
        ...
        for block in padded_input.chunks(18) {
        ...
        Output::from_u64_le_drop_4_bytes(vec![
            state.array[0][0],
            state.array[1][0],
            state.array[2][0],
            state.array[3][0],
...
mod tests {
    ...
    fn hash_works_on_special_characters() {
        ...
        assert_eq!(
            hasher.hash(&i1).output,
            "ef6ad6a3c062bc9f3afae2608b2441fdd4a342ce4be1fddaf9452758"
        );
        assert_eq!(
            hasher.hash(&i2).output,
            "f40e14a754c583754022c24acab43103666e9678351eb42ff0c39b98"
        );
    }
}
